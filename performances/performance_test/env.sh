...
ROS2_SDK_INSTALL_PATH="${ROS2_SDK_INSTALL_PATH:-/home/asoragna/ros2/ros2_sdk_ws/install}"

# Set the performance test workspace install path:
ROS2_PERFORMANCE_TEST_INSTALL_PATH="${ROS2_PERFORMANCE_TEST_INSTALL_PATH:-/home/asoragna/ros2/performance_ws/install}"

# Have the workspaces (ROS2 SDK and performance test) been compiled using --merge-install option?
MERGE_INSTALL="${MERGE_INSTALL:-false}"
...
if [ "$MERGE_INSTALL" = true ]; then
    ...
    ROS2_PERFORMANCE_TEST_LIBRARY_PATH="$ROS2_PERFORMANCE_TEST_INSTALL_PATH/lib"
else
    ...
    ROS2_PERFORMANCE_TEST_LIBRARY_PATH="$ROS2_PERFORMANCE_TEST_INSTALL_PATH/$PERFORMANCE_TEST_PKG/lib"
fi

# Check if the specified directories exist
if [ ! -d "$ROS2_PERFORMANCE_TEST_LIBRARY_PATH" ]; then
    echo "Directory $ROS2_PERFORMANCE_TEST_LIBRARY_PATH doesn't exist."
...
export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:$ROS2_SDK_LIBRARIES:$ROS2_PERFORMANCE_TEST_LIBRARY_PATH

# Specify where the C++ executables installed by this package are located
export ROS2_PERFORMANCE_TEST_EXECUTABLES_PATH="$ROS2_PERFORMANCE_TEST_LIBRARY_PATH/$PERFORMANCE_TEST_PKG"
